		...
		Task<Void> task = new Task<Void>() {
			...
			public Void call() throws IOException {
				...
				while (true) {
					...
					} else if (msg.startsWith("XAXRoundPoints")) {
					...
					} else if (msg.startsWith("XAXTotalPoints")) {
						msg = msg.substring(14);
					...
					}else if (msg.startsWith("XAXChangeTurn")) {
					...
					}else if (msg.startsWith("XAXRefreshBoard")) {
					...
					else if (msg.startsWith("XAXSynchronizeWheel")) {
					...
					else if (msg.startsWith("XAXSynchronizeDraggedWheel")) {
					...
					else if (msg.startsWith("XAXShowPassword")) {
						Platform.runLater(
								  () -> {
									 board.showPassword();
					...
					}else if (msg.startsWith("XAXRound")) {
					...
					else if (msg.startsWith("XAXSynchronizePassword")) {
					...
					else if (msg.startsWith("XAXRequestPassword")) {
						outputPrintWriter.println("XBXSynchronizePassword"+board.getCurrentPassword());
					}
					else if (msg.startsWith("XAXSynchronizeUsedPasswords")) {
						String l = msg.substring(27);
						passwordDataBase.addUsedPassword(l);
					}
					else if (msg.startsWith("XAXINFO")) {
...
//		board.checkPassword(password);		
	...
	public  synchronized void sendData() {
		outputPrintWriter.println("XBXRoundPoints" + game.getPlayerApoints());
		outputPrintWriter.println("XBXTotalPoints" + game.getPlayerApointsTotal());
		outputPrintWriter.println("XBXRound" + game.getRound());
	}
	/**
	 * Metoda wywo³uj¹ca zmianê tury 
	 */
	public  synchronized void changeTurn() {
		outputPrintWriter.println("XBXChangeTurn");
		wheel.switchTurn();
		goWhellButton.setDisable(true);
		givePasswordButton.setDisable(true);
		buyButton.setDisable(true);
	...
	public  synchronized void refreshBoard(String letter) {
		outputPrintWriter.println("XBXRefreshBoard" + letter);
	}
	/**
	...
	public synchronized void synchronizeWheel(double wheelVelocity) {
		outputPrintWriter.println("XBXSynchronizeWheel"+ wheelVelocity);
	}
	/**
	...
	public synchronized void synchronizeDraggedWheel(double wheelRotate) {
		outputPrintWriter.println("XBXSynchronizeDraggedWheel"+ wheelRotate);
	}
	/**
	...
	public  synchronized void changeRound() throws InterruptedException {
		...
		if (game.getRound()<6)
		{
		outputPrintWriter.println("XBXShowPassword");
		...
		else
		{
			...
			buyButton.setDisable(true);	
					...
					outputPrintWriter.println("XBXINFO"+ "PRZEGRYWASZ GRÊ. Przeciwnik odgad³ ostatnie has³o i ma wiêcej pieniêdzy.");
					rankUpdate();
					}
					else if (game.getPlayerApointsTotal() < game.getPlayerBpointsTotal())
					{
					infoLabel.setText("PRZEGRA£EŒ GRÊ:(");
					outputPrintWriter.println("XBXINFO"+ "Przeciwnik odgad³ ostatnie has³o ale to Ty WYGRYWASZ");
					rankUpdate();
					}
					else
					{
					infoLabel.setText("REMIS! Dzielicie siê pieniêdzmi.");	
					outputPrintWriter.println("XBXINFO"+ "REMIS! Dzielicie siê pieniêdzmi.");
					rankUpdate();
					}
		}
	}
	...
	public  synchronized void startNewRound()
	{
		...
		outputPrintWriter.println("XBXSynchronizeUsedPasswords"+board.getCurrentPassword());
		board.setCurrentPassword(passwordDataBase.getPassword());
		outputPrintWriter.println("XBXSynchronizePassword"+board.getCurrentPassword());
		changeTurn();
	...
	{
		outputPrintWriter.println("XBXRequestPassword");
	}
	/**
